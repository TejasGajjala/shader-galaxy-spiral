...
uniform float uFlare;           // 4-point diffraction spikes + soft bloom.
                                ...
                                // (uZoom < 0.10, full by 0.04), on ALL
...
// Window pushed LATER twice (0.22 -> 0.13 -> 0.10, judged on device):
// even with the squared ease-in below, earlier sparkle read as bright
// blobby stars well before the plunge. It lives in the plunge only now.
float flareRamp() {
    return smoothstep(0.10, 0.04, uZoom);
}
...
float starFlare(vec2 d, float dist, float radius, float reach, float hs, float ang, float fvis) {
    ...
    // (uZoom <= 0.04) the square is exactly 1, so the signed-off finale
    // is bit-identical.
...
vec2 starFieldLevel(vec2 p, float lvlScale, float seed, float keep, vec2 parVec, float partLo, float partHi, float ang, vec3 pxCtl, float wantAll, float armKeep, float resid) {
    ...
    for (int dy = -1; dy <= 1; dy++) {
        for (int dx = -1; dx <= 1; dx++) {
            ...
            bool flaring = uFlare > 0.001 && uZoom < 0.10 && hs > 0.25 && hs < 0.55;
