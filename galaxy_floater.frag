...
float floaterSheet(vec2 p, vec2 parVec, float hSheet, float seed, vec2 pxMM) {
    ...
    for (int dy = -1; dy <= 1; dy++) {
        for (int dx = -1; dx <= 1; dx++) {
            ...
            // Floaters glow, they don't flare. Diffraction spikes were tried
            // here and removed: deep in the dive they were never visibly
            // distinguishable from the main field's own flares, so they cost
            // code clarity for nothing on screen.
            if (dist < radius) {
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (inHole) {
        ...
        if (uFlatFloaters > 0.001) {
            ...
            starsV = max(starsV, floaterSheet(p, parRot, Hf, 1.0, pxCtl.xy) * 1.25);
        }
    }
